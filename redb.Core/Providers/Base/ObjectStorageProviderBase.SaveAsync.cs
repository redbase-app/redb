...
namespace redb.Core.Providers.Base
{
    ...
    public abstract partial class ObjectStorageProviderBase
    {
        ...
        private async Task<long> SaveAsyncDeleteInsertBulk<TProps>(IRedbObject<TProps> obj, IRedbUser user) where TProps : class, new()
        {
            ...
            // Top-level: wrap in deadlock retry (ORDER BY _id prevents most, but cross-table edge cases remain)
            if (isTopLevel)
            {
                return await DeadlockRetryHelper.ExecuteWithRetryAsync(async () =>
                {
                    return await SaveAsyncDeleteInsertBulkInternal(obj, user, isTopLevel: true);
                });
            }
            else
            {
                // Joined existing transaction — caller is responsible for retry
                return await SaveAsyncDeleteInsertBulkInternal(obj, user, isTopLevel: false);
            }
        }

        private async Task<long> SaveAsyncDeleteInsertBulkInternal<TProps>(IRedbObject<TProps> obj, IRedbUser user, bool isTopLevel) where TProps : class, new()
        {
