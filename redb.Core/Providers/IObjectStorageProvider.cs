...
namespace redb.Core.Providers
{
    ...
    public interface IObjectStorageProvider
    {
        ...
        /// <summary>
        /// Acquire row-level lock on objects by ID within the current transaction.
        /// Must be called inside an active transaction (<see cref="IRedbContext.BeginTransactionAsync"/>
        /// or <see cref="IRedbContext.ExecuteAtomicAsync(Func{Task})"/>).
        /// PostgreSQL: SELECT ... FOR UPDATE. MSSQL: SELECT ... WITH (UPDLOCK, ROWLOCK).
        /// </summary>
        /// <param name="objectIds">IDs of objects to lock</param>
        Task LockForUpdateAsync(params long[] objectIds);
    }
}
