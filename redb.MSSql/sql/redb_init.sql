...
-- Enable row-versioning for READ COMMITTED to eliminate reader/writer deadlocks
DECLARE @dbname NVARCHAR(256) = DB_NAME();
DECLARE @rcsi NVARCHAR(500) = N'ALTER DATABASE ' + QUOTENAME(@dbname) + N' SET READ_COMMITTED_SNAPSHOT ON;';
EXEC sp_executesql @rcsi;
GO
...
CREATE TABLE [dbo].[_users](
    ...
    CONSTRAINT [IX__users_login] UNIQUE ([_login])
)
GO
...
CREATE TABLE [dbo].[_users_roles](
    ...
    CONSTRAINT [FK__users_roles__users] FOREIGN KEY ([_id_user]) REFERENCES [_users]([_id]) ON DELETE NO ACTION
)
GO
...
CREATE TABLE [dbo].[_permissions](
    ...
    CONSTRAINT [FK__permissions__users] FOREIGN KEY ([_id_user]) REFERENCES [_users]([_id]) ON DELETE NO ACTION
)
GO
...
CREATE INDEX [IX__users__name] ON [dbo].[_users]([_name])
...
ALTER TABLE [dbo].[_users] ADD CONSTRAINT [FK__users__configuration] 
    FOREIGN KEY ([_id_configuration]) REFERENCES [_objects]([_id]) ON DELETE NO ACTION
GO
...
-- =====================================================
-- TRIGGER: Protect system users from deletion and modification
-- Prohibits deletion of users with ID -1, 0, 1
-- IDs 0 (sys), -1 (default): fully immutable (no changes allowed)
-- Login (_login) is immutable for ALL users
-- =====================================================

IF EXISTS (SELECT * FROM sys.triggers WHERE name = 'TR__users__protect_system')
DROP TRIGGER [dbo].[TR__users__protect_system]
GO

CREATE TRIGGER [dbo].[TR__users__protect_system]
ON [dbo].[_users]
...
BEGIN
    ...
    BEGIN
        -- DELETE operation: prohibit for system users
        IF EXISTS (SELECT 1 FROM deleted WHERE [_id] IN (0, 1, -1))
        BEGIN
            RAISERROR('Cannot delete system user', 16, 1);
            RETURN;
        END

        -- Manual cascade (replaces ON DELETE CASCADE removed from FK constraints
        -- to allow INSTEAD OF trigger on _users table — MSSQL restriction)
        DELETE ur FROM [dbo].[_users_roles] ur
        INNER JOIN deleted d ON ur.[_id_user] = d.[_id];

        DELETE p FROM [dbo].[_permissions] p
        INNER JOIN deleted d ON p.[_id_user] = d.[_id];

        DELETE u FROM [dbo].[_users] u
        INNER JOIN deleted d ON u.[_id] = d.[_id];
        RETURN;
    END
    ...
    BEGIN
        -- Prohibit changing ID
        IF EXISTS (SELECT 1 FROM deleted d INNER JOIN inserted i ON d.[_id] != i.[_id])
        BEGIN
            RAISERROR('Cannot change user ID', 16, 1);
            RETURN;
        END
        ...
        IF EXISTS (
            ...
            INNER JOIN inserted i ON d.[_id] = i.[_id]
            WHERE d.[_id] IN (0, -1)
              AND (
                  d.[_login] != i.[_login] OR d.[_password] != i.[_password] OR d.[_name] != i.[_name]
                  OR ISNULL(d.[_phone], '') != ISNULL(i.[_phone], '')
                  OR ISNULL(d.[_email], '') != ISNULL(i.[_email], '')
                  OR d.[_enabled] != i.[_enabled]
                  OR ISNULL(d.[_note], '') != ISNULL(i.[_note], '')
              )
        )
        BEGIN
            RAISERROR('Cannot modify system user with ID 0 or -1', 16, 1);
            RETURN;
        END
        ...
        IF EXISTS (
            SELECT 1 FROM deleted d
            INNER JOIN inserted i ON d.[_id] = i.[_id]
            WHERE d.[_login] != i.[_login]
        )
        BEGIN
            RAISERROR('Cannot change user login', 16, 1);
            RETURN;
        END
        ...
        UPDATE u SET
            u.[_login] = i.[_login],
            u.[_password] = i.[_password],
            u.[_name] = i.[_name],
            u.[_phone] = i.[_phone],
            u.[_email] = i.[_email],
            u.[_date_register] = i.[_date_register],
            u.[_date_dismiss] = i.[_date_dismiss],
            u.[_enabled] = i.[_enabled],
            u.[_key] = i.[_key],
            u.[_code_int] = i.[_code_int],
            u.[_code_string] = i.[_code_string],
            u.[_code_guid] = i.[_code_guid],
            u.[_note] = i.[_note],
            u.[_hash] = i.[_hash],
            u.[_id_configuration] = i.[_id_configuration]
        FROM [dbo].[_users] u
        INNER JOIN inserted i ON u.[_id] = i.[_id];
    END
END
GO
...
BEGIN
    ...
    -- Clear configuration references (replaces ON DELETE SET NULL removed from FK constraints
    -- to allow INSTEAD OF trigger on _users table — MSSQL restriction)
    UPDATE u SET u.[_id_configuration] = NULL
    FROM [dbo].[_users] u
    INNER JOIN deleted d ON u.[_id_configuration] = d.[_id];

    UPDATE r SET r.[_id_configuration] = NULL
    FROM [dbo].[_roles] r
    INNER JOIN deleted d ON r.[_id_configuration] = d.[_id];
...
BEGIN
    ...
    -- Turn NOCOUNT OFF and delete temp rows to produce correct rowcount for ExecuteNonQueryAsync
    -- #to_delete has exactly the same row count as total deleted from _objects
    SET NOCOUNT OFF;
    ...
    FROM (
        SELECT 
            CASE 
                ...
                -- Use zero-padding for numeric types to preserve correct sort order
                ELSE
                    N'(SELECT TOP 1 CASE
                        WHEN v._String IS NOT NULL THEN v._String
                        WHEN v._Long IS NOT NULL THEN RIGHT(''00000000000000000000'' + CAST(v._Long AS NVARCHAR(20)), 20)
                        WHEN v.[_Double] IS NOT NULL THEN RIGHT(''0000000000000000000000000'' + REPLACE(CAST(v.[_Double] AS NVARCHAR(50)), ''.'', ''~''), 25)
                        WHEN v._Numeric IS NOT NULL THEN RIGHT(''000000000000000000000000000000'' + REPLACE(CAST(v._Numeric AS NVARCHAR(50)), ''.'', ''~''), 30)
                        WHEN v._DateTimeOffset IS NOT NULL THEN CONVERT(NVARCHAR(50), v._DateTimeOffset, 127)
                        WHEN v._Boolean IS NOT NULL THEN CAST(CAST(v._Boolean AS INT) AS NVARCHAR(1))
                        ELSE NULL END
                       FROM _values v 
...
BEGIN
    ...
    SET @condition = CASE @operator
        ...
        WHEN '$in' THEN 
            CASE 
                WHEN @db_type = 'ListItem' THEN N'v._listitem IN (SELECT CAST([value] AS BIGINT) FROM OPENJSON(N''' + @safe_value + N'''))'
        ...
        WHEN '$notIn' THEN 
            CASE 
                WHEN @db_type = 'ListItem' THEN N'v._listitem NOT IN (SELECT CAST([value] AS BIGINT) FROM OPENJSON(N''' + @safe_value + N'''))'
        ...
        WHEN '$arrayContains' THEN 
            CASE 
                WHEN @db_type = 'ListItem' THEN N'v._listitem = ' + @value
                WHEN @db_type = 'Boolean' THEN N'v._Boolean = ' + CASE WHEN @value = 'true' THEN '1' ELSE '0' END
                WHEN @db_type = 'Long' THEN N'v._Long = ' + @value
                WHEN @db_type = 'Double' THEN N'v.[_Double] = ' + @value
                WHEN @db_type = 'Numeric' THEN N'v._Numeric = ' + @value
    ...
    BEGIN
        ...
        END;
    END
    -- ListItem direct: search by ID (Status = 123, Status $in [1,2,3])
    ...
    BEGIN
        ...
        IF @operator = '$exists'
        BEGIN
            -- $exists true = field exists (record in _values), $exists false = field NOT exists
            IF @value = 'true'
                SET @result = N'EXISTS (SELECT 1 FROM _values v ' +
                    N'WHERE v._id_object = ' + @table_alias + N'._id ' +
                    N'AND v._id_structure = ' + CAST(@structure_id AS NVARCHAR(20)) + N' ' +
            ...
            ELSE
                SET @result = N'NOT EXISTS (SELECT 1 FROM _values v ' +
                    ...
                    N'AND v._array_index IS NULL)';
        END
        ELSE IF @operator = '$ne' AND @value IS NOT NULL AND @value <> 'null'
        BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    BEGIN
                        ...
                        SET @base_cond = CASE @base_op
                            WHEN '$eq' THEN 
                                CASE WHEN @base_type = 0 THEN @table_alias + N'.' + @base_col + N' IS NULL'
                                    WHEN @is_datetime_col = 1 
                                    THEN @table_alias + N'.' + @base_col + N' = CONVERT(datetimeoffset, ''' + @safe_base_val + N''', 127)'
                                    ELSE @table_alias + N'.' + @base_col + N' = N''' + @safe_base_val + N''''
                                END
                            WHEN '$ne' THEN 
                                CASE WHEN @base_type = 0 THEN @table_alias + N'.' + @base_col + N' IS NOT NULL'
                                    WHEN @is_datetime_col = 1 
                                    THEN @table_alias + N'.' + @base_col + N' <> CONVERT(datetimeoffset, ''' + @safe_base_val + N''', 127)'
                                    ELSE @table_alias + N'.' + @base_col + N' <> N''' + @safe_base_val + N''''
                                END
                ...
                ELSE
                BEGIN
                    -- JSON null → IS NULL (OPENJSON returns @type=0 and @value=NULL for JSON null)
                    IF @type = 0
                        INSERT INTO @conditions (condition) VALUES (
                            @table_alias + N'.' + @base_col + N' IS NULL'
                        );
                    ELSE
                    BEGIN
...
BEGIN
    ...
    BEGIN
        ...
        IF @base_value_id IS NULL RETURN NULL;
    ...
    BEGIN
        ...
        IF @dict_base_id IS NULL RETURN NULL;
...
BEGIN
    ...
    IF NOT EXISTS(SELECT 1 FROM _values WHERE _id_object = @object_id)
        RETURN @base_json + N',"properties":null}';
    ...
    BEGIN
        ...
        SET @sql = N'
            SELECT @result = (
                ...
                FROM _objects o
                JOIN (
                    SELECT obj._id, ROW_NUMBER() OVER (' + @order_clause + N') AS _sort_order
                    FROM _objects obj
                    ' + @final_where + N'
                    ' + @order_clause;
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            -- No head record = array/dictionary property is null (not empty []/{})
            -- Without this, @base_array_value_id IS NULL fallback matches wrong records
            IF @base_array_value_id IS NULL
            BEGIN
                FETCH NEXT FROM structure_cursor INTO @structure_id, @field_name, @field_order, @collection_type,
                                                      @is_array, @is_dictionary, @type_name, @db_type, @type_semantic;
                CONTINUE;
...
BEGIN
    ...
    IF NOT EXISTS(SELECT 1 FROM #all_values WHERE [_id_object] = @object_id)
    BEGIN
        SET @result = @base_json + N',"properties":null}';
        IF @is_root_call = 1 DROP TABLE #all_values;
        RETURN;
    END
