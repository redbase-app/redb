...
- **`redb.Export` — `SQLitePCLRaw.lib.e_sqlite3` 2.1.10** ([GHSA-2m69-gcr7-jv3q], высокий).
  Приезжал транзитивно через `Microsoft.Data.Sqlite` 9.0.3. В `redb.SQLite` эта дыра была закрыта
  пином давно, а `redb.Export` прошёл мимо защиты. `Microsoft.Data.Sqlite` поднят до 10.0.0 плюс
  тот же пин `lib.e_sqlite3` 3.50.3.
- **`redb.CLI` теперь требует .NET 10.** Раньше он собирался под `net8.0` и вставал на любой
  рантайм .NET 8 и новее. Роллфорвард работает только вверх, поэтому на машине с одним лишь
  .NET 8 или .NET 9 тул больше не установится — нужен .NET 10.
