...
### Changed
- **`PropsSaveStrategy` renamed to `PropsSaveStrategy`** — users frequently asked
  ...
  - `PropsSaveStrategy` enum → `PropsSaveStrategy`
  - `RedbServiceConfiguration.PropsSaveStrategy` property → `PropsSaveStrategy`
  - JSON/appsettings key `"PropsSaveStrategy"` → `"PropsSaveStrategy"`
    (**breaking**: update `appsettings.json` / environment variables if set explicitly)
  - `Tsak:Redb:PropsSaveStrategy` config key → `Tsak:Redb:PropsSaveStrategy`
