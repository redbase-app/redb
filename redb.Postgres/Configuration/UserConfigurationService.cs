...
namespace redb.Postgres.Configuration
{
    /// <summary>
    ...
    public class UserConfigurationService : IUserConfigurationService
    {
        ...
        // Cache of effective configurations (userId -> (config, expireTime))
        ...
        private async Task<EffectiveUserConfiguration> BuildEffectiveConfigurationAsync(long userId)
        {
            ...
            // Step 4: Load user role configurations
            ...
            // Step 5: Load user personal configuration
        ...
        private void ApplyBaseConfiguration(EffectiveUserConfiguration effective)
        {
            ...
            effective.ListCacheSize = 5000; // Can be added to RedbServiceConfiguration
            ...
            effective.MaxRequestsPerMinute = null; // Not in base configuration
            effective.MaxBatchSize = 1000; // Default value
            effective.PropsSaveStrategy = _baseConfiguration.PropsSaveStrategy;
            ...
            int priority)
        {
            ...
            if (source.PropsSaveStrategy.HasValue)
            {
                target.PropsSaveStrategy = source.PropsSaveStrategy.Value;
                target.Sources.Add(new ConfigurationSource
                {
                    ParameterName = nameof(source.PropsSaveStrategy),
                    Source = sourceName,
                    Priority = priority,
                    Value = source.PropsSaveStrategy.Value.ToString()
                });
            }
        }
        ...
        public async Task SetUserConfigurationAsync(long userId, long? configId)
        {
            ...
            await _context.ExecuteAsync(
                "UPDATE _users SET _id_configuration = $1 WHERE _id = $2", (object?)configId ?? DBNull.Value, userId);
        ...
        public async Task SetRoleConfigurationAsync(long roleId, long? configId)
        {
            ...
            await _context.ExecuteAsync(
                "UPDATE _roles SET _id_configuration = $1 WHERE _id = $2", (object?)configId ?? DBNull.Value, roleId);
