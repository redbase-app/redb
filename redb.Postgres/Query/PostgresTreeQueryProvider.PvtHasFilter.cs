...
/// <see cref="PostgresTreeQueryProvider.ExecuteTreeToListAsync"/> with a
...
public partial class PostgresTreeQueryProvider
{
    ...
    private async Task<long[]> FindIdsViaPvtAsync(TreeFilter filter, long[]? scopeRootIds)
    {
        ...
        var treeIdsLiteral = scopeRootIds == null || scopeRootIds.Length == 0
            ? "NULL::bigint[]"
            : "ARRAY[" + string.Join(",", scopeRootIds.Select(id => id.ToString(CultureInfo.InvariantCulture))) + "]::bigint[]";
        ...
        var invocation = "SELECT pvt_build_query_sql("
            ...
            + ", $1::jsonb, NULL, 0, NULL::jsonb, "
            + maxDepth.ToString(CultureInfo.InvariantCulture)
            + ", false, '" + sourceMode + "', " + treeIdsLiteral
            + ", false, true) AS \"Value\"";
