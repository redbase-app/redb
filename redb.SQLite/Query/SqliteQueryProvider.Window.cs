...
/// <summary>
/// PVT-backed window functions for free SQLite. Generates the SELECT
...
public partial class SqliteQueryProvider
{
        ...
        int? skip)
    {
        ...
        var invocation = "SELECT pvt_build_window_sql("
            ...
            + ", $1, $2, NULL, $3, $4, 'flat', NULL, NULL, 1, 1) AS \"Value\"";
    ...
    /// SQLite has no native WINDOW-clause sharing at this layer.
    /// </summary>
        ...
        JsonElement? frameNode)
    {
        ...
        foreach (var f in selectFields)
        {
            ...
            {
                ["field"] = SqliteQueryProvider.NormalizePvtFieldName(f.FieldPath, f.IsBaseField)
            };
        ...
        var partitionEntries = partitionBy
            ...
            {
                ["field"] = SqliteQueryProvider.NormalizePvtFieldName(p.FieldPath, p.IsBaseField)
            })
            .ToList();
        var orderEntries = orderBy
            .Select(o => new Dictionary<string, object>
            {
                ["field"] = SqliteQueryProvider.NormalizePvtFieldName(o.FieldPath, o.IsBaseField),
